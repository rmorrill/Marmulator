%%%% DATA LOAD
function eyetracker_calibration_fcn_simplified(session_file_full)
...
sf = load(session_file_full);
eyetrack = sf.eyetrack; 
calib = sf.calib; 
calib_settings = sf.calib_settings; 
...
for i = 1:n_calib_pts
    plot(calib.pts(i, 1), calib.pts(i,2), 'Color', cols(i,:), 'Marker', 's', 'MarkerSize', 20, 'LineWidth', 1);
    hold on
...
%set(0, 'defaulttextinterpreter', 'none')
title(tc1, session_file, 'interpreter', 'none'); 
...
%% write coeffs into box
    function writeCoeffsTextIntoBox(coeff_X, coeff_Y, coeff_Proj)
        ...
        for i = 1:length(coeff_X)
            text(0.1, tts*(4-i), sprintf('cX(%d) = %0.1f', i, coeff_X(i)));
            text(0.5, tts*(4-i), sprintf('cY(%d) = %0.1f', i, coeff_Y(i)));
            if nargin == 3
                text(0.8, tts*(4-i), sprintf('cProj(%d) = %0.1f', i, coeff_Proj(i)));
            end
            axis tight
        end
        axis tight
